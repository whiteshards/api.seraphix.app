...
export const updateKeyHwid = async (keysystemId, keyValue, hwid, discordId = undefined) => {
  const db = getDatabase();
      ...
      for (let i = 0; i < session.keys.length; i++) {
        if (session.keys[i].value === keyValue) {
          ...
          if (discordId !== undefined) {
            updateFields[`keysystems.$[ks].keys.${sessionId}.keys.${i}.discord_id`] = discordId.toString();
          }
